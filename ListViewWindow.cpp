		...
		if( dwFileSize != INVALID_FILE_SIZE )
		{
			...
			if( ReadFile( hFile, lpszFileText, dwFileSize, NULL, NULL ) )
			{
				...
				while( lpszLine )
				{
					...
					lvItem.iSubItem	= LIST_VIEW_WINDOW_URL_COLUMN_ID;
...
BOOL ListViewWindowSetItemTextEx( int nWhichItem, int nWhichSubItem, LPCTSTR lpszItemText, DWORD dwMaximumTextLength )
{
	BOOL bResult = FALSE;

	// Set item text
	if( ListViewWindowSetItemText( nWhichItem, nWhichSubItem, lpszItemText, dwMaximumTextLength ) )
	{
		// Successfully set item text

		// Ensure that item is visible
		SendMessage( g_hWndListView, LVM_ENSUREVISIBLE, ( WPARAM )nWhichItem, ( LPARAM )FALSE );

		// Update list view window
		UpdateWindow( g_hWndListView );

	} // End of successfully set item text
